...
source_schema = "LRK"
source_name ="export_opendata_lrk"
...
if SourceFileType =='csv':
    df = spark.read.format("csv").option("header","true").option("sep",f"{ColumnDelimiter}").load(f"{SourceFilePath}"f"{SourceFileName}")
    display(df)
...
for table_columns in df.columns:
    if table_columns in columns_to_be_masked:
        cols.append(F.regexp_replace(f"{table_columns}", '(?<!^).(?!$)', '*').alias(f"{table_columns}_Masked"))
    elif table_columns in columns_to_be_encrypted:
        cols.append(base64(expr(f"aes_encrypt({table_columns}, '{super_secret_key}', 'ECB')")).alias(f"{table_columns}_Encrypted"))
    else:
        cols.append(expr(f"{table_columns}"))  
...
df_final.write \
    ...
    .saveAsTable(f"{source_schema}_{source_name}")
...
# Write encrypted and masked
parquet_table_name = 'opendata_lrk_encrypted.parquet'
df_final.write.mode("overwrite").format("parquet").save("Files/LRK/" + parquet_table_name)
...
def encrypt(df, columns_to_be_masked, columns_to_be_encrypted):
    cols = []
